    ...
    <div class="modal-content doclib-modal-content" style="width:min(720px, 92vw);max-height:85vh;background:var(--bg);">
      <div class="modal-header">
        <h4>
          ...
          ${_t('Email')}
          <span id="email-lib-unread-badge" class="email-lib-unread-badge" role="button" tabindex="0" title="${_t('Show unread emails')}" style="display:none"></span>
      ...
      <div class="modal-body" style="display:flex;flex-direction:column;gap:10px;overflow:hidden;">
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              <select class="memory-sort-select" id="email-lib-folder" style="flex:1;min-width:0;text-overflow:ellipsis;">
                <option value="INBOX">${_t('Inbox')}</option>
              </select>
              ...
              <select class="memory-sort-select" id="email-lib-filter" style="display:none;">
                ...
                <option value="unread">${_t('Unread')}</option>
                <option value="favorites">${_t('Favorites')}</option>
                <option value="undone">${_t('Undone')}</option>
                <option value="reminders">${_t('Reminders')}</option>
                <option value="unanswered">${_t('Unanswered')}</option>
                <option value="pending_30d">${_t('Pending · 30d')}</option>
                <option value="stale_30d">${_t('Stale · >30d')}</option>
                <optgroup label="${_t('Tags')}">
                  <option value="tag:urgent">${_t('Urgent')}</option>
                  <option value="tag:reply-soon">${_t('Reply soon')}</option>
                  <option value="tag:spam">${_t('Spam')}</option>
                  <option value="tag:newsletter">${_t('Newsletter')}</option>
                  <option value="tag:marketing">${_t('Marketing')}</option>
                </optgroup>
              </select>
              <div class="email-filter-picker" id="email-filter-picker" style="flex:1;min-width:0;position:relative;">
                <button type="button" class="email-filter-btn" id="email-filter-btn" aria-haspopup="listbox" aria-expanded="false">
                  <span class="email-filter-current"><span class="email-filter-icon"></span><span class="email-filter-label">${_t('All')}</span></span>
              ...
              <button class="memory-toolbar-btn email-filter-select-btn" id="email-lib-select-btn"><svg class="memory-select-btn-icon" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:3px;"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3" fill="currentColor" stroke="none"/></svg>${_t('Select')}</button>
              <button class="memory-toolbar-btn email-filter-refresh-btn" id="email-lib-refresh-btn" title="${_t('Refresh')}">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="${_t('Permanently delete Odysseus reminder emails')}">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              <div class="email-lib-chip-bar memory-search-input" id="email-lib-chip-bar" style="width:100%;padding-right:96px;padding-left:26px;display:flex;align-items:center;flex-wrap:wrap;gap:4px;cursor:text;min-height:30px;position:relative;">
                ...
                <input type="text" id="email-lib-search" placeholder="${_t('Search by name or text')}" autocomplete="off" style="flex:1;min-width:80px;border:0;outline:none;background:transparent;color:inherit;font:inherit;padding:0;position:relative;top:-1px;" />
              </div>
              ...
              <button class="memory-toolbar-btn email-undone-toggle email-undone-toggle-inline" id="email-undone-btn" title="${_t('Show only emails not marked as done (undone)')}">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="${_t('Show Odysseus reminder emails')}">
              ...
              <button class="memory-toolbar-btn email-attach-toggle email-attach-toggle-inline" id="email-attach-btn" title="${_t('Show only emails with attachments')}">
          ...
          <div id="email-lib-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
            <label class="memory-bulk-check-all" style="position:relative;top:0px;"><input type="checkbox" id="email-lib-select-all"> ${_t('All')}</label>
            <span id="email-lib-selected-count" style="position:relative;top:1px;">${_t('0 Selected')}</span>
            <button class="memory-toolbar-btn" id="email-lib-bulk-actions" style="position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>${_t('Actions')} <span style="opacity:0.55;font-size:9px;">\u25BC</span></button>
            <button class="memory-toolbar-btn" id="email-lib-bulk-delete" style="position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>${_t('Delete')}</button>
            <button class="memory-toolbar-btn" id="email-lib-bulk-cancel" title="${_t('Cancel (Esc)')}" style="margin-left:4px;padding:3px 6px;position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
          </div>
          <div id="email-lib-grid" class="doclib-grid"></div>
          <button class="email-lib-fab" id="email-lib-fab" type="button" aria-label="${_t('New email')}">
            ...
            <span class="email-lib-fab-label">${_t('New')}</span>
          </button>
        </div>
      </div>
    </div>
