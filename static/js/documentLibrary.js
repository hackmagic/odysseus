...
// ---- Library animation helpers ----
  ...
  function libraryRenderGrid() {
    ...
    if (_libraryDocs.length === 0) {
      if (_librarySearch || _libraryActiveLanguage) {
        grid.innerHTML = '<div class="doclib-empty">' + window.__('No documents match your search.') + '</div>';
      } else {
        ...
        grid.innerHTML =
          '<div class="doclib-empty" style="display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;">' +
            '<span>' + window.__('No documents yet') + '</span>' +
            '<span style="opacity:0.7;font-size:11px;">' +
              '<a href="#" data-doclib-import style="color:var(--accent,var(--red));text-decoration:underline;">' + window.__('Import') + _impIco + '</a>' +
              ' &middot; ' + window.__('or create one in a session') +
            '</span>' +
          '</div>';
        ...
        <div class="modal-body" style="display:flex;flex-direction:column;gap:10px;overflow:hidden;">
          <div id="doclib-panel-chats" data-doclib-panel="chats" class="admin-card" style="display:none;flex:1;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">
              <h2 style="margin:0;padding:0;line-height:1;">${window.__('Chats')} <span id="doclib-chats-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>
            </div>
            <p class="memory-desc doclib-desc">${window.__('All active chat sessions. Click to open.')}</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                <select class="memory-sort-select" id="doclib-chats-sort">
                  ...
                  <option value="most-messages">${window.__('Most messages')}</option>
                  <option value="alpha">A\u2013Z</option>
                </select>
                <button class="memory-toolbar-btn" id="doclib-chats-select-btn">${window.__('Select')}</button>
                <button class="memory-toolbar-btn" id="doclib-chats-tidy-btn" title="${window.__('AI tidy: delete junk sessions and organize into folders')}"><svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:2px;"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg> ${window.__('Tidy')}</button>
              </div>
              <input type="text" id="doclib-chats-search" placeholder="${window.__('Search chats...')}" class="memory-search-input" />
            ...
            <div id="doclib-chats-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              <label class="memory-bulk-check-all" style="position:relative;top:0px;left:-1px;"><input type="checkbox" id="doclib-chats-select-all" style="position:relative;top:0px;"> ${window.__('All')}</label>
              <span id="doclib-chats-selected-count">${window.__('0 Selected')}</span>
          ...
          <div id="doclib-panel-archive" data-doclib-panel="archive" class="admin-card" style="display:none;flex:1;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">
              <h2 style="margin:0;padding:0;line-height:1;position:relative;top:2px;">${window.__('Archive')} <span id="doclib-arc-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>
            </div>
            <p class="memory-desc doclib-desc" style="position:relative;top:0.5px;">${window.__('Archived sessions. Restore to make active again.')}</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                ...
                <button class="memory-toolbar-btn" id="doclib-arc-select-btn">${window.__('Select')}</button>
              </div>
              <input type="text" id="doclib-arc-search" placeholder="${window.__('Search archive...')}" class="memory-search-input" />
            ...
            <div id="doclib-arc-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              <label class="memory-bulk-check-all" style="position:relative;top:0px;left:1px;"><input type="checkbox" id="doclib-arc-select-all"> ${window.__('All')}</label>
              <span id="doclib-arc-selected-count">${window.__('0 Selected')}</span>
              <button class="memory-toolbar-btn" id="doclib-arc-bulk-restore" style="position:relative;top:-3px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>${window.__('Restore')}</button>
              <button class="memory-toolbar-btn danger" id="doclib-arc-bulk-delete"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>${window.__('Delete')}</button>
              <button class="memory-toolbar-btn" id="doclib-arc-bulk-cancel" title="${window.__('Cancel (Esc)')}" style="margin-left:4px;padding:3px 6px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
            </div>
          ...
          <div id="doclib-panel-research" data-doclib-panel="research" class="admin-card" style="display:none;flex:1;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;margin-top:10px;">
              <h2 style="margin:0;padding:0;line-height:1;">${window.__('Research')} <span id="doclib-research-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>
            </div>
            <p class="memory-desc doclib-desc" style="position:relative;top:-1px;">${window.__('Completed deep research reports. Click to view.')}</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                <select class="memory-sort-select" id="doclib-research-sort">
                  <option value="recent">${window.__('Recent')}</option>
                  <option value="oldest">${window.__('Oldest')}</option>
                  <option value="most-sources">${window.__('Most sources')}</option>
                  <option value="alpha">A\u2013Z</option>
                </select>
                <button class="memory-toolbar-btn" id="doclib-research-select-btn">${window.__('Select')}</button>
                <button class="memory-toolbar-btn" id="doclib-research-tidy-btn" title="${window.__('Tidy: delete research with no sources or empty reports')}">${window.__('Tidy')}</button>
              </div>
              <input type="text" id="doclib-research-search" placeholder="${window.__('Search research...')}" class="memory-search-input" />
            </div>
            <div id="doclib-research-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              <label class="memory-bulk-check-all" style="position:relative;top:0px;left:1px;"><input type="checkbox" id="doclib-research-select-all"> ${window.__('All')}</label>
              <span id="doclib-research-selected-count">${window.__('0 Selected')}</span>
          ...
          <div data-doclib-panel="documents" class="admin-card" style="flex:1;display:flex;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">
              <h2 style="margin:0;padding:0;line-height:1;">${window.__('Documents')} <span id="doclib-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>
              <button class="memory-toolbar-btn" id="doclib-import-file-btn" title="${window.__('Import files from disk')}" style="margin-left:auto;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:2px;"><polyline points="7 10 12 5 17 10"/><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="21" x2="19" y2="21"/></svg> ${window.__('Import')}</button>
              <button class="memory-toolbar-btn" id="doclib-create-btn" title="${window.__('Create new blank document')}"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg> ${window.__('Create')}</button>
            </div>
            <p class="memory-desc doclib-desc">${window.__('Open documents in a session, clone to a new or import new files.')}</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                <select class="memory-sort-select" id="doclib-sort">
                  <option value="recent">${window.__('Recent')}</option>
                  <option value="oldest">${window.__('Oldest')}</option>
                  <option value="edits">${window.__('Most edits')}</option>
                  <option value="alpha">A\u2013Z</option>
                </select>
                <button class="memory-toolbar-btn" id="doclib-select-btn" title="${window.__('Select documents')}">${window.__('Select')}</button>
                <button class="memory-toolbar-btn" id="doclib-tidy-btn" title="${window.__('Tidy: remove empty / junk / duplicate documents')}">${window.__('Tidy')}</button>
              </div>
              <input type="text" id="doclib-search" placeholder="${window.__('Search titles & content...')}" class="memory-search-input" />
            ...
            <div id="doclib-bulk-bar" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              <label class="memory-bulk-check-all" style="position:relative;top:0px;left:1px;"><input type="checkbox" id="doclib-select-all" /> ${window.__('All')}</label>
              <span id="doclib-selected-count">${window.__('0 Selected')}</span>
              ...
              <button id="doclib-bulk-cancel" class="memory-toolbar-btn" title="${window.__('Cancel (Esc)')}" style="margin-left:4px;margin-right:4px;padding:3px 6px;position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
            </div>
            <div class="doclib-grid" id="doclib-grid"></div>
            <button class="doclib-load-more" id="doclib-load-more" style="display:none">${window.__('Load more')}</button>
          </div>
        </div>
      </div>
    `;
