...
function _startChat(url, mid, endpointId) {
  ...
  if (sessionModule) {
    ...
    uiModule.showError(window.__('Session module not loaded'));
  }
}
...
function _buildModelRow(mid, url, displayName, endpointId, offline, modelType) {
  ...
  handle.textContent = '\u22EE\u22EE';
  handle.title = window.__('Drag to reorder');
  ...
  fav.title = window.__('Toggle favorite');
  fav.addEventListener('click', (e) => {
    e.stopPropagation();
    const nowFav = _toggleFavorite(mid);
    fav.classList.toggle('active', nowFav);
    uiModule.showToast(nowFav ? window.__('Favorited') : window.__('Unfavorited'));
    refreshModels();
  });
  ...
  if (modelType === 'image') {
    ...
    badge.title = window.__('Image generation model');
  ...
  btn.type = 'button';
  btn.textContent = offline ? window.__('Offline') : (modelType === 'image' ? window.__('+ Image') : window.__('+ Chat'));
...
export async function refreshModels(force = false) {
  ...
  if (needsFetch) {
    ...
    try {
      ...
      console.error(e);
      box.textContent = '(' + window.__('scan failed') + ')';
      return;
    } finally {
      box.innerHTML = '';
    }
  ...
  try {
    ...
    if (_cachedItems && _cachedItems.length > 0) {
      _cachedItems.forEach(item => {
        ...
        const epName = item.endpoint_name || window.__('Unknown');
      ...
      Object.entries(endpoints).forEach(([epName, epModels]) => {
        ...
        if (multiEndpoints) {
          ...
          if (isOfflineEndpoint) {
            ...
            badge.textContent = window.__('(offline)');
        ...
        if (allHidden.length > 0) {
          ...
          showMoreBtn.textContent = window.__('Show') + ' ' + allHidden.length + ' ' + (allHidden.length === 1 ? window.__('more model') : window.__('more models'));
    ...
    if (totalModelCount >= 10) {
      ...
      searchBox.type = 'text';
      searchBox.placeholder = window.__('Search models...');
      ...
      searchBox.addEventListener('input', () => {
        ...
        if (searchResults.children.length === 0) {
          ...
          empty.textContent = window.__('No models match') + ' "' + searchBox.value.trim() + '"';
    ...
    if (!_cachedItems || _cachedItems.length === 0) {
      ...
      if (window._isAdmin) {
        noModels.innerHTML = '<span class="muted">' + window.__('No models found') + '</span><br>'
          + '<a href="#" onclick="document.getElementById(\'user-bar-admin\')?.click();return false;" class="accent-link">' + window.__('Open Admin to add endpoints') + '</a>'
          + '<br><span class="muted-sm">' + window.__('Type /setup for Local models or API setup.') + '</span>';
      } else {
        noModels.innerHTML = '<span class="muted">' + window.__('No models available') + '</span><br>'
          + '<span class="muted-sm">' + window.__('Ask an admin to configure model endpoints') + '</span>';
      }
      box.appendChild(noModels);
      // No endpoints yet: keep the welcome screen focused on first setup.
      const welcomeSub = document.getElementById('welcome-sub');
      if (welcomeSub) welcomeSub.innerHTML = window.__('Type') + ' <span class="setup-trigger-link" style="color:var(--accent,var(--red));font-weight:600;cursor:pointer;text-decoration:underline;" title="' + window.__('Click to launch setup') + '">/setup</span> ' + window.__('to get started.') + '';
      const welcomeTip = document.getElementById('welcome-tip');
      if (welcomeTip) welcomeTip.textContent = window.__('Type /setup, then choose Local models or API.');
    } else {
      // Configured installs should feel ready, not stuck in onboarding.
      const welcomeSub = document.getElementById('welcome-sub');
      if (welcomeSub) welcomeSub.textContent = window.__('Yours for the voyage.');
      const welcomeTip = document.getElementById('welcome-tip');
      if (welcomeTip) {
        const tips = window.innerWidth <= 768
          ? [
              window.__('Tip: Long-press a session for rename, delete, and memory options.'),
              window.__('Tip: Tap the eye icon for Nobody mode - no history saved.'),
              window.__('Tip: Switch to Agent mode when you want tools.'),
              window.__('Tip: Attach images or files using the + button next to the input.'),
            ]
          : [
              window.__('Tip: Press Ctrl+K to search across all your conversations.'),
              window.__('Tip: Press Ctrl+B to quickly toggle the sidebar.'),
              window.__('Tip: Shift-click the sidebar toggle to swap it to the other side.'),
              window.__('Tip: Drag and drop files onto the chat to attach them.'),
              window.__('Tip: Right-click a session for rename, delete, and memory options.'),
            ];
    ...
    console.error(e);
    box.textContent = '(' + window.__('render failed') + ': ' + e.message + ')';
  }
}
...
export async function refreshProviders() {
  ...
  sel.innerHTML = '<option disabled>' + window.__('Loading providers…') + '</option>';
  ...
  try {
    ...
    if (openai) {
      ...
      const opt = document.createElement('option');
      opt.value = '';
      opt.textContent = window.__('(OPENAI_API_KEY not set on server)');
      sel.appendChild(opt);
    }
  } catch (e) {
    console.error(e);
